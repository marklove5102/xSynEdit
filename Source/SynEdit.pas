...
//-- xOpt
    ...
    FLayoutCache: TSynLayoutCache;  // Cache for DirectWrite layouts (optimization)
    FHitTestMetricsBuffer: array of TDwriteHitTestMetrics;  // Reusable buffer for selection rendering (optimization)
...
//-- CodeFolding
  ...
  protected
    ...
    procedure ShowDefaultContextMenu(MousePos: TPoint);
...
//-- CodeFolding
//++ xOpt (Extended Options: auto keyboard layout, char filter)
    property xOpt: TSynXOpt read fXOpt write fXOpt;
//-- xOpt
...
begin
  ...
  begin
    ...
    begin
      ...
      // Use cached layout for non-ASCII text measurement
      SetString(S, P, P2-P);
      Layout := FLayoutCache.GetLayout(FTextFormat, S, P2-P, MaxInt, fTextHeight);
...
begin
  ...
  begin
    ...
    // Use cached layout for text measurement
    SetString(TempStr, P, P2-P);
    Layout := FLayoutCache.GetLayout(FTextFormat, TempStr, P2-P, MaxInt, fTextHeight);
    if P2 < PCol then
    begin
      P := P2;
...
begin
  ...
  // Use cached layout for text position validation
  SetString(TempStr, PStart, P - PStart + 1);
  Layout := FLayoutCache.GetLayout(FTextFormat, TempStr,
    P - PStart + 1, MaxInt, fTextHeight);
...
//-- CodeFolding
//++ xOpt
  fXOpt := TSynXOpt.Create;
//-- xOpt
  // Create layout cache for performance optimization
  FLayoutCache := TSynLayoutCache.Create(200);  // Cache up to 200 layouts
...
begin
  ...
  // Clear layout cache - font changed, all cached layouts are invalid
  if Assigned(FLayoutCache) then
    FLayoutCache.Clear;
...
function TCustomSynEdit.GetSelText: string;
  ...
  begin
    ...
    if SrcLen >= DstLen then
      ...
      P := PWideChar(Result);
      Move(S[Index], P^, Count * SizeOf(Char));
      Inc(P, Count);
  ...
  // TStringBuilder version for large selections (50-90% faster)
  procedure AppendWithBuilder(var SB: TStringBuilder; const S: string; Index, Count: Integer);
  var
    SrcLen: Integer;
    ActualCount: Integer;
  begin
  ...
  procedure AppendPaddedWithBuilder(var SB: TStringBuilder; const S: string; Index, Count: Integer);
  var
    SrcLen: Integer;
    ActualCount: Integer;
    PadCount: Integer;
...
begin
  if not SelAvail then
    ...
    // Use TStringBuilder for large multi-line selections (50-90% faster)
    ...
    case fActiveSelectionMode of
      smNormal:
        ...
        else if UseStringBuilder then
        begin
          // TStringBuilder path for large selections
          ...
          // Original PWideChar path for small selections
          TotalLen := Max(0, Length(Lines[First]) - ColFrom + 1);
          for i := First + 1 to Last - 1 do
            Inc(TotalLen, Length(Lines[i]));
          Inc(TotalLen, ColTo - 1);
...
begin
  ...
  // xOpt: Process character (filter + case conversion)
  // Only process printable characters (> #31), allow control chars like Backspace, Enter etc.
  if Assigned(fXOpt) and (Key > #31) then
  begin
    Key := Char(fXOpt.ProcessChar(Key));
    if Key = #0 then
      Exit;
  end;
...
var
  ...
  begin
    ...
    // Use cached layout for tab glyph
    TabLayout := FLayoutCache.GetLayout(FTextFormat, SynTabGlyph, 1,
      Round(X2 - X1), fTextHeight);
        ...
        begin
          ...
          begin
            RT.DrawRectangle(HintRect,
              TSynDWrite.SolidBrush(fCodeFolding.CollapsedLineColor),
              FCurrentPPI/96);
            // Use cached layout for hint mark (3 spaces)
            Layout := FLayoutCache.GetLayout(FTextFormat,
              StringOfChar(SynSpaceGlyph, 3), 3, HintRect.Width, HintRect.Height);
...
begin
  // Cache BlockBegin/BlockEnd to avoid repeated property calls
  ...
  begin
    ...
    // Create the text layout (using cache for performance)
    if LastChar > 0 then
    begin
      LayoutWidth := ClientWidth - FTextOffset - XRowOffset;
      // Use cached layout instead of creating new one every time
      Layout := FLayoutCache.GetLayout(FTextFormat,
      ...
      if not (eoShowLigatures in FOptions) or (Line = CaretY) then
        ...
        Layout.SetTypography(typNoLigatures, 1, LastChar - FirstChar + 1);
    end;
    ...
    begin
      ...
      begin
        ...
        // Optimization: Reuse pre-allocated buffer instead of SetLength per row
        if Length(FHitTestMetricsBuffer) < Integer(RangeCount) then
          SetLength(FHitTestMetricsBuffer, RangeCount);
        Layout.IDW.HitTestTextRange(SelFirst - 1, SelLast - SelFirst + 1,
          FTextOffset + XRowOffset, YRowOffset(Row), FHitTestMetricsBuffer[0], RangeCount, RangeCount);
        if SameValue(fSelectedColor.Alpha, 1)  then
        begin
          for I := 0 to RangeCount -1  do
          begin
            Layout.SetFontColor(SelFG, FHitTestMetricsBuffer[I].textPosition + 1, FHitTestMetricsBuffer[I].length);
            RT.FillRectangle(Rect(Round(FHitTestMetricsBuffer[I].left), YRowOffset(Row),
              SelEndX(FHitTestMetricsBuffer[I].Left, FHitTestMetricsBuffer[I].Width, SelLast, I, RangeCount),
              YRowOffset(Row + 1)), TSynDWrite.SolidBrush(SelBG));
          end;
          RangeCount := 0;
        end;
  ...
  procedure InsertText;
    ...
    begin
      ...
      begin
         ...
         if fXOpt.Limits.Enabled and (fXOpt.Limits.MaxLineLength > 0) then
         begin
           if fXOpt.Limits.GetLineLength(Str) > fXOpt.Limits.MaxLineLength then
           begin
             case fXOpt.Limits.LengthMode of
               lmChars:
                 Str := Copy(Str, 1, fXOpt.Limits.MaxLineLength);
               lmBytes:
                 while fXOpt.Limits.GetLineLength(Str) > fXOpt.Limits.MaxLineLength do
...
var
  Len: Integer;
begin
  Len := Min(Text.Length, Msg.TextMax - 1);
  if Len > 0 then
    Move(Text[1], Msg.Text^, Len * SizeOf(Char));
  Msg.Text[Len] := #0;
  Msg.Result := Len;
end;
...
begin
  ...
  // xOpt: Restore keyboard layout on exit
  if Assigned(fXOpt) then
    fXOpt.HandleEditorExit;
...
begin
  ...
  // xOpt: Switch keyboard layout on enter
  if Assigned(fXOpt) then
    fXOpt.HandleEditorEnter;
...
begin
  ...
  // Handle system context menu if enabled in xOpt
  if (Msg.Msg = WM_CONTEXTMENU) and (PopupMenu = nil) and
     FxOpt.Menu.UseSystemMenu then
  begin
    ShowDefaultContextMenu(Point(SmallInt(LoWord(Msg.LParam)), SmallInt(HiWord(Msg.LParam))));
    Exit;
  end;
...
begin
  ...
  // Check line length limit (xOpt.Limits)
  if not SelAvail and not fXOpt.CanAddChar(LineText) then
    Exit;
...
begin
  ...
  begin
    ...
    // Use cached layout for text width measurement
    SetString(TempStr, P, P2-P);
    Layout := FLayoutCache.GetLayout(FTextFormat, TempStr, P2-P, MaxInt, fTextHeight);
    Inc(Result, Round(Layout.TextMetrics.width));
    P := P2;
...
// vertical caret movement or selection
      ...
      ecInsertLine,
      ecLineBreak:
        if not ReadOnly and fXOpt.CanAddLine(Lines.Count) then begin
          BeginUndoBlock;
          try
            if SelAvail then
              SetSelText('');
...
var
  ...
  Layout: TSynTextLayout;
begin
  // Use cached layout for plugin/handler painting
  Layout := FLayoutCache.GetLayout(FTextFormat, S, S.Length, MaxInt, fTextHeight);
...
procedure TCustomSynEdit.ShowDefaultContextMenu(MousePos: TPoint);
const
  CMD_UNDO = 1; CMD_REDO = 2; CMD_CUT = 3; CMD_COPY = 4; CMD_PASTE = 5;
  CMD_DELETE = 6; CMD_SELECTALL = 7; CMD_SELECTWORD = 8;
  CMD_DELETELINE = 9;
  CMD_INDENT = 10; CMD_UNINDENT = 11;
  CMD_UPPERCASE = 12; CMD_LOWERCASE = 13;
  CMD_GOTOLINE = 14; CMD_FIND = 15; CMD_REPLACE = 16;
var
  hMenu: THandle;
  Cmd: Integer;
  Flags: LongWord;
  ...
  function GetText(const EnText, RuText: string): string;
  begin
    if IsRussian then Result := RuText else Result := EnText;
  end;
...
begin
  ...
  try
    // Undo/Redo
    if smiUndo in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if not CanUndo then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_UNDO, PChar(GetText('&Undo'#9'Ctrl+Z', '&Отменить'#9'Ctrl+Z')));
    end;
    if smiRedo in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if not CanRedo then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_REDO, PChar(GetText('&Redo'#9'Ctrl+Y', '&Повторить'#9'Ctrl+Y')));
    end;

    if smiSep1 in FxOpt.Menu.VisibleItems then AppendMenu(hMenu, MF_SEPARATOR, 0, nil);

    // Cut/Copy/Paste/Delete
    if smiCut in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly or not SelAvail then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_CUT, PChar(GetText('Cu&t'#9'Ctrl+X', '&Вырезать'#9'Ctrl+X')));
    end;
    if smiCopy in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if not SelAvail then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_COPY, PChar(GetText('&Copy'#9'Ctrl+C', '&Копировать'#9'Ctrl+C')));
    end;
    if smiPaste in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly or not CanPaste then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_PASTE, PChar(GetText('&Paste'#9'Ctrl+V', 'Вст&авить'#9'Ctrl+V')));
    end;
    if smiDelete in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly or not SelAvail then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_DELETE, PChar(GetText('&Delete'#9'Del', '&Удалить'#9'Del')));
    end;

    if smiSep2 in FxOpt.Menu.VisibleItems then AppendMenu(hMenu, MF_SEPARATOR, 0, nil);

    // Select operations
    if smiSelectAll in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if Lines.Count = 0 then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_SELECTALL, PChar(GetText('Select &All'#9'Ctrl+A', 'В&ыделить всё'#9'Ctrl+A')));
    end;
    if smiSelectWord in FxOpt.Menu.VisibleItems then
      AppendMenu(hMenu, MF_STRING, CMD_SELECTWORD, PChar(GetText('Select &Word', 'Выделить &слово')));

    if smiSep3 in FxOpt.Menu.VisibleItems then AppendMenu(hMenu, MF_SEPARATOR, 0, nil);

    // Line operations
    if smiDeleteLine in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_DELETELINE, PChar(GetText('Delete Li&ne'#9'Ctrl+Y', 'Удалить стр&оку'#9'Ctrl+Y')));
    end;

    if smiSep4 in FxOpt.Menu.VisibleItems then AppendMenu(hMenu, MF_SEPARATOR, 0, nil);

    // Indent operations
    if smiIndent in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_INDENT, PChar(GetText('&Indent'#9'Tab', '&Отступ'#9'Tab')));
    end;
    if smiUnindent in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_UNINDENT, PChar(GetText('Unin&dent'#9'Shift+Tab', 'Убрать отст&уп'#9'Shift+Tab')));
    end;

    if smiSep5 in FxOpt.Menu.VisibleItems then AppendMenu(hMenu, MF_SEPARATOR, 0, nil);

    // Case conversion
    if smiUppercase in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly or not SelAvail then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_UPPERCASE, PChar(GetText('UPPERCASE', 'ВЕРХНИЙ РЕГИСТР')));
    end;
    if smiLowercase in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly or not SelAvail then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_LOWERCASE, PChar(GetText('lowercase', 'нижний регистр')));
    end;

    if smiSep6 in FxOpt.Menu.VisibleItems then AppendMenu(hMenu, MF_SEPARATOR, 0, nil);

    // Navigation
    if smiGotoLine in FxOpt.Menu.VisibleItems then
      AppendMenu(hMenu, MF_STRING, CMD_GOTOLINE, PChar(GetText('&Go to Line...'#9'Ctrl+G', 'Перейти к &строке...'#9'Ctrl+G')));

    if smiSep7 in FxOpt.Menu.VisibleItems then AppendMenu(hMenu, MF_SEPARATOR, 0, nil);

    // Search
    if smiFind in FxOpt.Menu.VisibleItems then
      AppendMenu(hMenu, MF_STRING, CMD_FIND, PChar(GetText('&Find...'#9'Ctrl+F', '&Найти...'#9'Ctrl+F')));
    if smiReplace in FxOpt.Menu.VisibleItems then
    begin
      Flags := MF_STRING;
      if ReadOnly then Flags := Flags or MF_GRAYED;
      AppendMenu(hMenu, Flags, CMD_REPLACE, PChar(GetText('R&eplace...'#9'Ctrl+H', '&Заменить...'#9'Ctrl+H')));
    end;

    Cmd := Integer(TrackPopupMenu(hMenu, TPM_LEFTALIGN or TPM_RETURNCMD or TPM_RIGHTBUTTON,
      MousePos.X, MousePos.Y, 0, Handle, nil));

    case Cmd of
      CMD_UNDO: Undo;
      CMD_REDO: Redo;
      CMD_CUT: CutToClipboard;
      CMD_COPY: CopyToClipboard;
      CMD_PASTE: PasteFromClipboard;
      CMD_DELETE: SelText := '';
      CMD_SELECTALL: SelectAll;
      CMD_SELECTWORD: CommandProcessor(ecSelWord, ' ', nil);
      CMD_DELETELINE: CommandProcessor(ecDeleteLine, ' ', nil);
      CMD_INDENT: CommandProcessor(ecBlockIndent, ' ', nil);
      CMD_UNINDENT: CommandProcessor(ecBlockUnindent, ' ', nil);
      CMD_UPPERCASE: CommandProcessor(ecUpperCase, ' ', nil);
      CMD_LOWERCASE: CommandProcessor(ecLowerCase, ' ', nil);
      CMD_GOTOLINE: if Assigned(FxOpt.Menu.OnGotoLine) then FxOpt.Menu.OnGotoLine(Self);
      CMD_FIND: if Assigned(FxOpt.Menu.OnFind) then FxOpt.Menu.OnFind(Self);
      CMD_REPLACE: if Assigned(FxOpt.Menu.OnReplace) then FxOpt.Menu.OnReplace(Self);
    end;
  finally
    DestroyMenu(hMenu);
  end;
...
begin
  ...
  // Cache UpperCase calls for performance (optimization)
  var UpperSelText := UpperCase(SelText);
  var UpperFindText := UpperCase(FindDialog.FindText);
  if (UpperSelText = UpperFindText) and not (frReplace in FindDialog.Options) then
