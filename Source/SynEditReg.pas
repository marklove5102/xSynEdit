...
type
  { Property Editor for Lang property - shows keyboard layouts in dropdown }
  TSynXOptLangProperty = class(TStringProperty)
  public
...
{ TSynXOptLangProperty }

function TSynXOptLangProperty.GetAttributes: TPropertyAttributes;
begin
  Result := [paValueList, paSortList];
end;

procedure TSynXOptLangProperty.GetValues(Proc: TGetStrProc);
var
  Layouts: TStringList;
  I: Integer;
begin
  ...
  try
    TSynXOpt.GetInstalledLayouts(Layouts);
    for I := 0 to Layouts.Count - 1 do
    begin
      // Format is "DisplayName=LayoutID"
      // Show full "DisplayName (LayoutID)" in dropdown, but store only LayoutID
      Proc(Layouts[I]);
    end;
  finally
    Layouts.Free;
...
// SynEdit highlighters
  RegisterComponents(SYNS_HighlightersPage, [
    ...
    TSynTclTkSyn, TSynGWScriptSyn, TSynRubySyn, TSynRustSyn, TSynUNIXShellScriptSyn, TSynLuaSyn,
    //database
  ...
  // Property Editors
  RegisterPropertyEditor(TypeInfo(string), TSynXOptKeyboard, 'Lang', TSynXOptLangProperty);
end;
