...
type
  // Hot path data structure - 16 bytes, cache-line friendly
  TSynAttributeData = record
    ...
    BackgroundDefault: TColor;    // 4 bytes (Total: 20 bytes)
  end;

  TSynHighlighterAttributes = class(TPersistent)
  private
    fData: TSynAttributeData;     // Inline data for hot path access
  ...
  public
    ...
    property Data: TSynAttributeData read fData; // Direct access for hot path
...
begin
  bChanged := False;
  if fData.Background <> Source.fData.Background then
  begin
    fData.Background := Source.fData.Background;
    bChanged := True;
  end;
  if fData.Foreground <> Source.fData.Foreground then
  begin
    fData.Foreground := Source.fData.Foreground;
    bChanged := True;
  end;
  if fData.Style <> Source.fData.Style then
  begin
    fData.Style := Source.fData.Style;
    bChanged := True;
  end;
  if bChanged then
    Changed;
...
begin
  inherited Create;
  fData.Background := clNone;
  fData.Foreground := clNone;
  fData.BackgroundDefault := clNone;
  fData.ForegroundDefault := clNone;
  fData.Style := [];
  fName := AName;
...
begin
  Result := fData.Background <> fData.BackgroundDefault;
end;

function TSynHighlighterAttributes.GetForegroundColorStored: Boolean;
begin
  Result := fData.Foreground <> fData.ForegroundDefault;
end;

function TSynHighlighterAttributes.GetFontStyleStored: Boolean;
begin
  Result := fData.Style <> fStyleDefault;
end;

procedure TSynHighlighterAttributes.InternalSaveDefaultValues;
begin
  fData.ForegroundDefault := fData.Foreground;
  fData.BackgroundDefault := fData.Background;
  fStyleDefault := fData.Style;
end;
...
function TSynHighlighterAttributes.GetBackground: TColor;
begin
  Result := fData.Background;
end;

function TSynHighlighterAttributes.GetForeground: TColor;
begin
  Result := fData.Foreground;
end;

function TSynHighlighterAttributes.GetStyle: TFontStyles;
begin
  Result := fData.Style;
end;
...
begin
  if fData.Background <> Value then
  begin
    fData.Background := Value;
    Changed;
  end;
end;
...
begin
  if (fData.Foreground <> Foreground) or (fData.Background <> Background) then
  begin
    fData.Foreground := Foreground;
    fData.Background := Background;
    Changed;
  end;
end;
...
begin
  if fData.Foreground <> Value then
  begin
    fData.Foreground := Value;
    Changed;
  end;
end;
