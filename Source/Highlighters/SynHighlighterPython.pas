...
-------------------------------------------------------------------------------}
{
@abstract(A Python 3.13+ language highlighter for SynEdit with modern syntax support)
@author(Olivier Deckmyn, converted to SynEdit by David Muir, modernized for Python 3.13+)
@created(unknown, converted to SynEdit on 2000-06-23)
@lastmod(2026-02-04)
Supports Python 3.10-3.13+ features:
- match/case pattern matching (3.10+)
- Walrus operator := (3.8+)
- f-strings with all prefixes (f"", fr"", rf"", etc.)
- async/await keywords
- Type hints syntax
- Modern built-in functions (aiter, anext, breakpoint, etc.)
- Code folding for all blocks including match/case
}
...
const
  ...
  // Python 3.13+ built-in functions and constants
  NONKEYWORDCOUNT = 82;
  NONKEYWORDS: array [1..NONKEYWORDCOUNT] of string =
    ...
    'Ellipsis',
    '__debug__',
    // Built-in functions (Python 3.13+)
    '__import__',
    'abs',
    'aiter',        // Python 3.10+
    'all',
...
procedure TSynPythonSyn.ColonProc;
begin
  inc(Run);
  ...
  begin
    inc(Run);
    fTokenID := tkSymbol;  // := operator
  end
  else
    fTokenID := tkSymbol;  // regular :
end;
...
var
  ...
  offset: Integer;
begin
  // Handle Python string prefixes: r, f, b, u and combinations
  // Valid: r"", f"", b"", fr"", rf"", br"", rb"" (Python 3.6+)
  offset := 1;

  // Check for two-letter combinations (fr, rf, br, rb)
  if CharInSet(FLine[Run + 1], ['r', 'R', 'f', 'F', 'b', 'B']) then
    offset := 2;

  temp := FLine[Run + offset];
  if temp = '''' then
  begin
    Inc(Run, offset);
    StringProc;
  ...
  case fRange of
    ...
    rsMultilineString3:
      ...
      case fLine[Run] of
        ...
        'r', 'R', 'f', 'F', 'b', 'B': PreStringProc;  // r"", f"", b"", and combinations
...
begin
  Result :=
    ...
    '"""Python 3.13+ sample code with modern features"""'#13#10 +
    #13#10 +
    'from typing import TypeAlias'#13#10 +
    'import sys'#13#10 +
    #13#10 +
    '# Type hints (PEP 484, 585)'#13#10 +
    'def greet(name: str, count: int = 1) -> None:'#13#10 +
    '    for i in range(count):'#13#10 +
    '        # f-string (Python 3.6+)'#13#10 +
    '        print(f"Hello, {name}! (#{i+1})")'#13#10 +
    #13#10 +
    '# Walrus operator := (Python 3.8+)'#13#10 +
    'if (n := len(sys.argv)) > 1:'#13#10 +
    '    print(f"Arguments: {n-1}")'#13#10 +
    #13#10 +
    '# Match-case (Python 3.10+)'#13#10 +
    'def process_command(cmd: str) -> str:'#13#10 +
    '    match cmd.lower():'#13#10 +
    '        case "start" | "run":'#13#10 +
    '            return "Starting..."'#13#10 +
    '        case "stop":'#13#10 +
    '            return "Stopping..."'#13#10 +
    '        case _:'#13#10 +
    '            return "Unknown command"'#13#10 +
    #13#10 +
    '# Async/await (Python 3.5+)'#13#10 +
    'async def fetch_data(url: str) -> dict:'#13#10 +
    '    # Simulated async operation'#13#10 +
    '    await asyncio.sleep(1)'#13#10 +
    '    return {"status": "ok", "url": url}'#13#10 +
    #13#10 +
