...
type
  ...
  private
    ...
    // Interval cache for FoldLineToRow optimization
    fCachedLine: Integer;
    fCachedRow: Integer;
...
begin
  ...
  fRanges.Capacity := 256; // Pre-allocate for typical file sizes

  fCollapsedState := TList<Integer>.Create;
  fCollapsedState.Capacity := 64; // Pre-allocate
  ...
  fFoldInfoList.Capacity := 512; // Info typically larger than ranges
...
procedure TSynFoldRanges.InvalidateCache;
begin
  Inc(fCacheVersion);
  fCachedLine := -1;
  fCachedRow := -1;
...
Var
  ...
  TypeToStackIndex: TDictionary<Integer, TList<Integer>>;  // O(1) lookup by FoldType
  LFI : TLineFoldInfo;
  PFoldRange : PSynFoldRange;
  i : Integer;
  Line : integer;
  TypeStack: TList<Integer>;
  StackIdx: Integer;
begin
  fRanges.Clear;
  ...
  try
    ...
    begin
        ...
        if LFI.Indent >= 0 then begin
          ...
          begin
            ...
            if (PFoldRange^.Indent >= LFI.Indent) then begin
              ...
              if TypeToStackIndex.TryGetValue(PFoldRange^.FoldType, TypeStack) then
              begin
                TypeStack.Remove(OpenFoldStack.List[i]);
                if TypeStack.Count = 0 then
                begin
                  TypeToStackIndex.Remove(PFoldRange^.FoldType);
                  TypeStack.Free;
                end;
              end;
      ...
      begin
        ...
        begin
          // O(1) lookup by FoldType instead of O(n) backward scan
          if TypeToStackIndex.TryGetValue(LFI.FoldType, TypeStack) and (TypeStack.Count > 0) then
          begin
            // Get the most recent (last) fold of this type
            StackIdx := TypeStack.Last;
