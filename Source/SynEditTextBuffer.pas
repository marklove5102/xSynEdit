...
type
  ...
  private
    ...
    FAllTextWidthsUnknown: Boolean;  // Global flag for batch invalidation (optimization)
    FDetectUTF8: Boolean;
...
begin
  if (Index >= 0) and (Index < FCount) then
  begin
    // Check global flag first (optimization for FontChanged)
    if FAllTextWidthsUnknown or (sfTextWidthUnknown in FList^[Index].FFlags) then
    begin
...
const
  // Threshold for parallel processing - sequential is faster for small files
...
var
  ...
  NeedRecalc: Boolean;
begin
  if (FMaxWidth > 0) and not FAllTextWidthsUnknown then
    Result := FMaxWidth
  else if FCount = 0 then
    Result := 0
  else
  begin
    NeedRecalc := FAllTextWidthsUnknown;
    // Use sequential processing for small files (faster due to no thread overhead)
    if FCount < PARALLEL_THRESHOLD then
    begin
      LocalMax := 0;
      for I := 0 to FCount - 1 do
      begin
        PRec := @FList^[I];
        if NeedRecalc or (sfTextWidthUnknown in PRec^.FFlags) then
        begin
          PRec^.FTextWidth := FTextWidthFunc(PRec^.FString);
          Exclude(PRec^.FFlags, sfTextWidthUnknown);
        end;
        if PRec^.FTextWidth > LocalMax then
          LocalMax := PRec^.FTextWidth;
      end;
      FMaxWidth := LocalMax;
    end
    else
    begin
      ...
      var
        LMaxW: Integer;
        LRec: PSynEditStringRec;
      begin
        LRec := @FList^[I];
        if NeedRecalc or (sfTextWidthUnknown in LRec^.FFlags) then
        begin
          LRec^.FTextWidth := FTextWidthFunc(LRec^.FString);
          Exclude(LRec^.FFlags, sfTextWidthUnknown);
        end;
        repeat
          LMaxW := FMaxWidth;
          if LRec^.FTextWidth <= LMaxW then
            break;
        until AtomicCmpExchange(FMaxWidth, LRec^.FTextWidth, LMaxW) = LMaxW;
      end);
    end;
    FAllTextWidthsUnknown := False;  // Clear global flag after recalculation
...
var
  Lines: TArray<string>;
  LineCount: Integer;
  P, Start, TextEnd: PChar;
  I: Integer;
begin
  if NewText = '' then
    Exit;

  // Optimized: Direct parsing without temporary TStringList
  // First pass: count lines
  LineCount := 1;
  P := PChar(NewText);
  TextEnd := P + Length(NewText);
  while P < TextEnd do
  begin
    if P^ = #13 then
    begin
  ...
  begin
    if P^ = #13 then
    begin
      SetString(Lines[I], Start, P - Start);
      Inc(I);
      Inc(P);
      if (P < TextEnd) and (P^ = #10) then
        Inc(P);
  ...
  // Last line (or only line if no line breaks)
  if I < LineCount then
    SetString(Lines[I], Start, P - Start);
...
begin
  // Optimized: Use global flag instead of O(n) loop through all lines
  // Individual line flags will be cleared lazily during GetTextWidth/GetMaxWidth
  FMaxWidth := -1;
  FAllTextWidthsUnknown := True;
end;
