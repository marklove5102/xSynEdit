...
begin
  ...
  try
    ...
    // Use TStringBuilder for efficient concatenation (optimization)
    var SB := TStringBuilder.Create;
    try
      for i := 0 to Chunks.Count -1 do
        SB.Append(Chunks[i]^.Str);
      Result := SB.ToString;
    finally
      SB.Free;
    end;
...
var
  ...
  SB: TStringBuilder;
Begin
  // Use TStringBuilder for efficient string building (optimization)
  SB := TStringBuilder.Create(Length(APrettyText));
  try
    i := 1;
      ...
      case APrettyText[i] of
        #1, #2:
          begin
            ...
            SB.Append('\color{').Append(ColorToString(Color)).Append('}');
