...
begin
  ...
  begin
    ...
    SetLength(aName, BufferSize div SizeOf(Char));
    if BufferSize > 0 then
      Read(aName[1], BufferSize);
...
begin
  with AStream do begin
    ...
    SetLength(aName, bufSize div SizeOf(Char));
    if bufSize > 0 then
      Read(aName[1], bufSize);
    Read(aPitch, SizeOf(aPitch));
    Read(aSize, SizeOf(aSize));
    Read(aStyle, SizeOf(aStyle));
