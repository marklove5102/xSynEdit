...
{$ENDREGION}
  ...
  TSynFontMetricsCacheKey = record
    ...
    constructor Create(const AFontName: string; AHeight: Integer; AWeight: DWRITE_FONT_WEIGHT;
      AStyle: DWRITE_FONT_STYLE; AGDINatural: Boolean);
  ...
  public
    ...
    class function GetTypography(Typography: TSynTypography): IDWriteTypography; static;
  end;
...
{ TSynFontMetricsCacheKey }

constructor TSynFontMetricsCacheKey.Create(const AFontName: string; AHeight: Integer;
  AWeight: DWRITE_FONT_WEIGHT; AStyle: DWRITE_FONT_STYLE; AGDINatural: Boolean);
begin
  FontName := AFontName;
  FontHeight := AHeight;
  FontWeight := AWeight;
  FontStyle := AStyle;
  UseGDINatural := AGDINatural;
end;

class operator TSynFontMetricsCacheKey.Equal(const A, B: TSynFontMetricsCacheKey): Boolean;
begin
  Result := (A.FontName = B.FontName) and
            (A.FontHeight = B.FontHeight) and
            (A.FontWeight = B.FontWeight) and
            (A.FontStyle = B.FontStyle) and
            (A.UseGDINatural = B.UseGDINatural);
end;

class operator TSynFontMetricsCacheKey.NotEqual(const A, B: TSynFontMetricsCacheKey): Boolean;
begin
  Result := not (A = B);
end;
...
function DWTextRange(startPosition: Cardinal; length: Cardinal): TDwriteTextRange; inline;
begin
...
function DWFontFeature(nameTag: DWRITE_FONT_FEATURE_TAG; parameter: Cardinal): TDwriteFontFeature; inline;
begin
...
var
  ...
  Baseline: Single;
  CacheKey: TSynFontMetricsCacheKey;
  CachedData: TSynFontMetricsData;
  FontFamilyStr: string;
  FontWeight: DWRITE_FONT_WEIGHT;
begin
  FUseGDINatural := AFont.Quality = TFontQuality.fqClearTypeNatural;
  FCharExtra := CharExtra;
  ...
  FontFamilyStr := FontFamilyName(DWFont);
  ...
  begin
    ...
    // Calculate base metrics (without Extra spacing)
    ...
    CachedData.CharWidth := Round(-GlyphMetrics.advanceWidth *
      AFont.Height / FontMetrics.designUnitsPerEm);
    CachedData.Baseline := Round(FontMetrics.ascent * (-AFont.Height) /
      FontMetrics.designUnitsPerEm);

    // Store in cache
    if TSynDWrite.FFontMetricsCache = nil then
      TSynDWrite.FFontMetricsCache := TDictionary<TSynFontMetricsCacheKey, TSynFontMetricsData>.Create;
  ...
  CheckOSError(TSynDWrite.DWriteFactory.CreateTextFormat(
    PChar(FontFamilyStr), nil,
    FontWeight, DWFontStyle, DWRITE_FONT_STRETCH_NORMAL,
...
class function TSynDWrite.GetTypography(Typography: TSynTypography): IDWriteTypography;
const
...
begin
  ...
  begin
    case Typography of
      typEmpty: Result := DWGetTypography([]);
      typDefault: Result := DWGetTypography(DefaultTypoFeatures);
      typNoLigatures: Result := DWGetTypography(TypoFeaturesNoLigatures);
    end;
...
begin
  ...
  FIDW.SetTypography(TSynDWrite.GetTypography(Typography), Range);
end;
...
finalization
  ...
  TSynDWrite.FTypographyCache.Free;
end.
